...
export class LoginService {
  ...
  correo: string = '';
  correoUserBD: string = environment.CORREO_USER_DB
  passwordUserBD: string = environment.PASSWORD_DB
  ...
  async login(rut: string, password: string): Promise<Usuario | undefined> {
    try {
      ...
      if (Array.isArray(usuarioResponse.body) && usuarioResponse.body.length > 0) {
        ...
        const decryptedPassword = this._encriptadorService.decrypt(usuario.password);
